	...
	public function		loadClass($className)
	{
		...
		if ($this->namespace === NULL || isset($namespace))
		{
			...
			if (($lastNsPos = strripos($className, $this->separator)) !== false)
			{
				...
				$fileName = str_replace($this->separator, DIRECTORY_SEPARATOR, $namespace) . DIRECTORY_SEPARATOR;
			}
			$fileName .= str_replace('_', DIRECTORY_SEPARATOR, $className) . $this->extension;
			$include = ($this->includePath !== NULL ? $this->includePath . DIRECTORY_SEPARATOR : '') . $fileName;
