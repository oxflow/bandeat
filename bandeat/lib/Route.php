...
class						Route
{
	...
	public function			__construct($name = '', $url = '', $controller = '', $action = 'index', $method = 'GET')
	{
		$this->setName($name);
		$this->setUrl($url);
	...
	{
		if (is_array($varsTypes) && count($varsTypes) === count($this->varsNames))
		{
			foreach ($varsTypes as $key => $val)
			{
				if (in_array($key, $this->varsNames))
					...
					throw new \LogicException(sprintf('Variable "%s" doesn\'t exists in Route'));
			}
		}
		return $this;
	}
