<div class="container">
	<div class="jumbotron">
		...
		if (!empty($comments))
		{
			...
			{
				$user = $userMod->getUser($comment['author']);
				if ($id == $user->id() OR ($admin > 1 AND $admin >= $user->admin()))
					$url = $link->getUrl('Blog', 'comment', array('id' => $comment['id'], 'action' => 'edit'));
				?>
				<div class="panel panel-default">
					<div class="panel-heading">
						...
						{
							?>
							<a href="<?php echo $url; ?>" style="float:right"><button type="button" class="btn btn-primary">Edit article</button></a>
							<div style="clear:both"></div>
							<?php
							unset($url);
						}
						if ($comment['edited'] > 0)
							echo '<br><em>Edited ' . $comment['edited'] . ' times, last time at ' . $comment['update'] . '.</em>';
						?>
					</div>
